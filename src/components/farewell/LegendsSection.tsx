...
import { legends, STUDENTS_PER_WALL, TOTAL_WALLS, type Legend } from "./legendsData";
...
export function LegendsSection() {
  ...
  const goToWall = useCallback((index: number) => {
    setCurrentWall(Math.max(0, Math.min(index, TOTAL_WALLS - 1)));
  }, []);
  ...
  const handleFindYourself = useCallback(() => {
    ...
    // Scroll into view after wall renders
    setTimeout(() => {
          ...
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex flex-col items-center gap-2">
              <span className="text-primary-foreground/60 dark:text-muted-foreground text-sm">
                Wall {currentWall + 1} of {TOTAL_WALLS} · Roll #{wallStart}–{wallEnd}
              </span>
              {/* Dots */}
              <div className="flex gap-2">
                {Array.from({ length: TOTAL_WALLS }, (_, i) => (
                  <button
                    key={i}
                    onClick={() => goToWall(i)}
            ...
            <Button
              ...
              size="icon"
              onClick={() => goToWall(currentWall + 1)}
              disabled={currentWall === TOTAL_WALLS - 1}
              className="text-primary-foreground/80 dark:text-foreground/80 hover:bg-card/20 disabled:opacity-30"
            >
